        ...
        <template x-if="activeEmployee">
            ...
            <div x-show="activeTab === 'profile'" class="p-3">
                ...
                <div class="mt-2 grid grid-cols-3 gap-1.5">
                    <button type="button" @click="setStatus(activeEmployee, 'active')" :disabled="saving" :aria-busy="saving ? 'true' : 'false'" class="h-8 rounded-md border border-slate-300 bg-white text-[10.5px] font-bold text-slate-700 hover:border-emerald-500 disabled:cursor-wait disabled:opacity-60">Active</button>
                    <button type="button" @click="setStatus(activeEmployee, 'inactive')" :disabled="saving" :aria-busy="saving ? 'true' : 'false'" class="h-8 rounded-md border border-slate-300 bg-white text-[10.5px] font-bold text-slate-700 hover:border-slate-500 disabled:cursor-wait disabled:opacity-60">Inactive</button>
                    <button type="button" @click="setStatus(activeEmployee, 'suspended')" :disabled="saving" :aria-busy="saving ? 'true' : 'false'" class="h-8 rounded-md border border-rose-300 bg-white text-[10.5px] font-bold text-rose-600 hover:bg-rose-50 disabled:cursor-wait disabled:opacity-60">Suspend</button>
                </div>
            </div>
                                ...
                                <tr>
                                    ...
                                    <template x-for="a in actions" :key="a">
                                        <td class="text-center">
                                            <button type="button" @click="togglePermission(activeEmployee, m, a)" :disabled="saving" :aria-busy="saving ? 'true' : 'false'"
                                    ...
                                    <td>
                                        <button type="button" x-show="isOverridden(activeEmployee, m)" @click="resetModuleToDefault(activeEmployee, m)" :disabled="saving" :aria-busy="saving ? 'true' : 'false'" class="text-[9.5px] font-bold text-amber-600 underline decoration-amber-300 underline-offset-2 disabled:cursor-wait disabled:opacity-60">Reset</button>
                                    </td>
                                </tr>
                            </template>
                        </tbody>
            ...
            <div x-show="activeTab === 'shifts'" class="p-3">
                <div class="grid grid-cols-3 gap-1.5">
                    <template x-for="[key, s] in Object.entries(shiftTypes)" :key="key">
                        <button type="button" @click="setShift(activeEmployee, key)" :disabled="saving" :aria-busy="saving ? 'true' : 'false'" :class="activeEmployee.shift === key ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-300 bg-white text-slate-700'" class="rounded-md border p-3 text-center disabled:cursor-wait disabled:opacity-60">
    ...
    <x-pos.dialog name="form" width="max-w-lg" title="Employee Details">
        ...
        <x-slot:footer>
            <div class="flex gap-2">
                ...
                <button type="button" @click="saveEmployee()" :disabled="saving || !draft.name?.trim() || !draft.email?.trim() || (draft.phone || '').trim().length < 10" :aria-busy="saving ? 'true' : 'false'" class="h-10 flex-1 rounded-md bg-slate-900 text-[12px] font-black uppercase tracking-wide text-white hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-300">
