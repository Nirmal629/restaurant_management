{{-- DiscountModal — whole bill or a single item, percentage or fixed, reason mandatory. --}}
<x-pos.dialog name="discount" width="max-w-lg" title="Apply Discount">
    <div class="space-y-3 p-4">
        ...
        <div>
            ...
            <select x-show="discountDraft.scope === 'item'" x-model="discountDraft.target"
                ...
                <template x-for="i in billableItems.filter(x => x.status === 'normal' || x.status === 'discounted')" :key="i.uid">
                    <option :value="i.uid" x-text="i.qty + '× ' + i.name + ' — ' + money(i.amount)"></option>
                </template>
            </select>
        </div>
        ...
        <div class="grid grid-cols-[1fr_auto] gap-1.5">
            <div class="relative">
                ...
                <span class="pos-num absolute right-3 top-1/2 -translate-y-1/2 text-[15px] font-bold text-slate-400" x-text="discountDraft.mode === 'pct' ? '%' : '₹'"></span>
            </div>
            <div class="flex gap-1">
                <template x-for="v in (discountDraft.mode === 'pct' ? [5, 10, 15, 20] : [50, 100, 200, 500])" :key="v">
                    <button type="button" @click="discountDraft.value = v" class="pos-num h-12 w-12 rounded-md border border-slate-300 bg-white text-[12px] font-bold text-slate-700 hover:border-slate-900" x-text="v"></button>
                </template>
            </div>
        </div>
        ...
        <div class="rounded-md border border-slate-300 bg-slate-50 p-3">
            <dl class="space-y-1 text-[12px]">
                <div class="flex justify-between"><dt class="text-slate-500" x-text="discountDraft.scope === 'item' ? 'Item amount' : 'Subtotal'"></dt><dd class="pos-num font-semibold" x-text="money(discountBase)"></dd></div>
                <div class="flex justify-between"><dt class="text-slate-500">Discount</dt><dd class="pos-num font-semibold text-amber-700" x-text="'− ' + money(discountPreview)"></dd></div>
                <div class="flex justify-between border-t border-dashed border-slate-300 pt-1">
                    <dt class="font-bold text-slate-900">New amount</dt>
                    <dd class="pos-num text-[15px] font-black text-slate-900" x-text="money(discountBase - discountPreview)"></dd>
                </div>
            </dl>
            <div x-show="discountNeedsApproval" class="mt-2 flex items-start gap-2 rounded border border-amber-400 bg-amber-50 p-2">
                ...
                <p class="text-[11.5px] font-semibold leading-snug text-amber-900">
                    Manager approval required — <span class="pos-num" x-text="Math.round(discountPct) + '%'"></span> exceeds the
                    <span class="pos-num" x-text="operator.discountLimitPct + '%'"></span> limit for this role.
                </p>
            </div>
        </div>
    </div>
    ...
    <x-slot:footer>
        <div class="flex items-center gap-2">
            <button type="button" x-show="discountDraft.scope === 'bill' && billDiscount" @click="clearBillDiscount(); back()"
                    class="h-10 rounded-md border border-slate-300 bg-white px-3 text-[12px] font-bold text-rose-600 hover:border-rose-500">Remove</button>
            ...
            <button type="button" @click="applyDiscount()" :disabled="!(Number(discountDraft.value) > 0) || !discountDraft.reason || (discountDraft.scope === 'item' && !discountDraft.target)"
