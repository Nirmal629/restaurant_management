...
<x-pos.dialog name="coupon" width="max-w-md" title="Apply Coupon">
    ...
    <x-slot:footer>
        <div class="flex gap-2">
            ...
            <button type="button" @click="applyCoupon()" :disabled="saving || !couponDraft.code.trim()" class="h-10 flex-1 rounded-md bg-slate-900 text-[12px] font-black uppercase tracking-wide text-white hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-300">Apply</button>
        </div>
    </x-slot:footer>
</x-pos.dialog>
