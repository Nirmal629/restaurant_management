...
<body x-data="inventoryApp" x-cloak class="adm-root adm-shell bg-slate-100 text-slate-900 antialiased">
    ...
    <div class="adm-main">
        ...
        <x-admin.tabs :tabs="['stock' => 'Stock', 'ingredients' => 'Ingredients', 'ledger' => 'Stock Ledger', 'wastage' => 'Wastage', 'count' => 'Stock Count', 'low' => 'Low Stock']" active="tab" />
                        ...
                        <tbody>
                            <template x-for="i in pagedIngredients" :key="i.id">
                                <tr>
                                    ...
                                    <td><x-admin.badge expr="i.status" /></td>
                                    <td @click.stop><button type="button" @click="openIngredientForm(i)" class="grid h-7 w-7 place-items-center rounded border border-slate-200 text-slate-500 hover:border-slate-900"><x-pos.icon name="edit" class="h-3.5 w-3.5" /></button></td>
                                </tr>
                            </template>
                        </tbody>
                    </table>
        ...
        <template x-if="tab === 'wastage'">
            <div class="adm-table-wrap bg-white">
                <table class="adm-table">
                    <thead><tr><th>Wastage #</th><th>Ingredient</th><th>Qty</th><th>Reason</th><th>Cost</th><th>Employee</th><th>Date</th></tr></thead>
                    <tbody>
                    ...
                    <tbody>
                        <template x-for="i in lowStockList" :key="i.id">
                            <tr>
                                ...
                                <td class="text-slate-600" x-text="i.supplier"></td>
                                <td><x-admin.badge expr="i.status" /></td>
                                <td @click.stop><a href="{{ route('purchases') }}" class="rounded-md bg-slate-900 px-2.5 py-1.5 text-[10.5px] font-bold uppercase tracking-wide text-white hover:bg-slate-800">Create PO</a></td>
                            </tr>
                        </template>
                    </tbody>
                </table>
